...
`IntatisMessageContentView` 是很薄的显示/生命周期 adapter。Markdown grammar、AST、inline attributed content、代码块和 table layout 归经审计的 `SwiftStreamingMarkdown` 派生包；Intatis 不遍历或改写 AST，也不写 Markdown parser、lexer、TeX 或 layout。Intatis 只决定消息角色是否允许 rich、是否低于 64 KiB、何时申请 parse permit、是否仍是最新 revision、上游 document 何时可替换 raw Text，以及整段 raw SwiftUI `Text` 何时允许重新投影。raw 投影是 Markdown 无关的 MainActor latest-only 状态机：一个 facade lifetime 内保留最新源，100 ms fixed-window leading/trailing throttle 不随每个 token 重置；activation/reentry、非 append correction/truncation 和 final 同步精确直出，timer 与 task generation 双重拒绝旧发布。75 ms 初始候选在正式 replay 中有 2/20 轮 interaction p95 超过冻结门，因此不能恢复。派生包基于 Microsoft v0.6.0 exact commit，升级到 `swift-markdown`/`swift-cmark` 0.8，使用 public `@concurrent ... sending` parse boundary；返回的 non-Sendable `RenderableDocument` 不越过 MainActor ownership。该派生包的 Swift 6 strict Debug/Release 和 generic iOS consumers 已通过；传递的 swift-markdown manifest 仍使用 Swift 5 language mode，不能把整张依赖图表述为 Swift 6 strict-clean。

第一版故意关闭图片、citations、文字动画、LaTeX 与语法高亮；代码块显示完整可选择纯文本、语言标签、原生 Copy 与水平滚动，table copy/download actions 不进入 UI。链接点击只允许 `http`、`https`、`mailto`。因此本版本没有 JavaScriptCore/highlight.js 运行、iosMath/font payload、远程图片请求或模型输出 `eval`。零 completed-document cache、零 paragraph native-view cache；这一选择减少旧历史 session 的常驻原生 view graph，之后只有得到测量证据才可重新引入 cache。

当前 Intatis dependency 声明已删除旧 MarkdownUI/NetworkImage/swift-cmark0.5/iosMath/HighlightSwift 和 vendor highlight 资源，NOTICE/ThirdPartyNotices 已改为 Microsoft derivative + swift-markdown0.8 + swift-cmark0.8。完整可构建派生包现 vendored 于 `Vendor/SwiftStreamingMarkdown`，根 `Package.swift` 使用仓内相对路径；Microsoft MIT `LICENSE` 与永久 patch ledger 和源码一起由 Intatis 根 Git revision 固定，不再要求单独发布 remote fork。`swift-markdown` 0.8 与 `swift-cmark` 0.8 仍是 exact-pinned remote dependencies，首次无缓存解析需要网络；完全离线供应链不在本轮范围。
...
2026-07-18 latest-build GUI/Computer Use 验收为 `FAIL / ABORTED`，整体 release `NO-GO`：三份 validation 实例误并存，Force Quit 对主实例显示 129.63 GB application memory；CPU diagnostic incident `FA228932-2C40-4AC2-A0C2-62EF41342B4A` 记录 160 秒内 90 秒 CPU 与 sampled footprint 109.16 MB→803.30 MB，重栈含 SwiftUICore/AttributeGraph/lazy layout/`ParagraphView`/`SelectionOverlay`。缺少 malloc stack/heap graph，根因/最终 retaining edge 为 `UNKNOWN`。早期 Lazy interaction/xctrace 不能覆盖该 adverse evidence。事故后 vendor strict 44/44（Debug/Release）、SharedUI 21/21、iOS test compile-only、current Debug/Release builds、两个 Archive 与六-app static audit 通过；这些均不能替代受控单实例 GUI memory/CPU + selection/copy/accessibility 复验。
...
Recovery 的 startup gate 只检查 durable `@main` exact resolution 与 reviewer/control-plane readiness；完成 Goal recovery 后即可 resume pending tasks。普通 worker unresolved 不得把整个 scheduler 冻结：它自己的 queued invocation 会在 provider request 前 exact-resolution fail closed 并 durable 结算为 failed，撤销 task lease、清除 busy fence后才可显式 rebind；无关 agent 继续运行。Non-empty CLI session 若根本没有 durable `@main`，启动流程不能用今天的 default/workspace 自动创建；host 只能通过 `/agent restore-main <path> <profile-id>` 走普通 reviewed attach，再显式 `/auto` 冻结并启动控制面。
...
Cowork session open/new -> CoworkProjectSettingsStore loads/saves per-session project settings
  ...
  -> brand-new empty session: future-agent default exact binding + primary workspace -> restricted durable bootstrap of fixed @main
  -> resolve @main exact profile; freeze this process's reviewer/GoalVerifier control-plane provider
...
- Reviewer 与 GoalVerifier 是控制面，不跟随普通 agent rebind；GUI/CLI 首次以 `@main` exact binding 冻结其进程内 provider。当前没有独立 route lease 或跨 trust-domain 专用审批，不能从 control-plane freeze 推导出这些尚未实现的能力。
...
| `TaskContract` / `TaskReportPayload`（AgentInvocation execution layer） | 单次 root/child agent invocation 契约与结构化回报；不是用户可见 WorkTask | `task_created/assigned/queued/started/completed/failed/cancelled` 既有事件；queue/start/terminal 记录 attempt | 契约记录 kind、issuer/assignee/objective/roleHint/deliverable/lease、reply mode、timeout、maxAttempts，并以可选字段绑定 WorkTask/ContinuationRun/Goal 和 exact inference binding；普通用户消息与每个 Goal run 均创建 root invocation，legacy child/mailbox wake 继承 causal parent Goal/run scope。执行前 frozen binding 必须与 live roster 一致；完成只产生 candidate result；追加字段保持旧 JSONL 可解码 |
...
| `CoworkProjectSettings` / `CoworkProjectInfo` | Cowork project-mode session metadata 与右侧 inspector 投影 | UserDefaults `intatis.cowork.projectSettings.<sessionID>`；GUI 运行时投影 | 保存主 agent 名称、未来新 agent 的 exact inference default、默认 permission profile、可选 token budget、workspace path/bookmark/agent 归属；旧 provider/model 仅为兼容镜像，不得重写现有 agent；不保存 secret；fresh empty session 对用户明确选择的 primary workspace 使用受限 `@main` bootstrap admission，非空恢复/后续扩展仍走普通 `agent.attach`；project info 只读投影安全 profile/resolution/agent/workspace 状态；direct multi-root tool context 尚未实现 |
...
- **工具执行反馈**：AgentLoop 对未知工具、权限拒绝、工具抛错分别写入结构化 `tool_result` observation，并在执行前追加 `agent_status(tool)`。模型给出的 raw arguments 在 `.tool_call` 持久化前先分类；unknown/invalid 与作为 inference-control surface 的全部 `spawn_agent` inputs 只记录 bounded redacted placeholder + count/redacted，且不写 raw-value digest；schema-valid 非控制工具也先 secret-scrub/限长，只有未脱敏/未截断的 canonical 参数才可附加 digest。随后，已知工具在权限判断和执行前会校验参数必须是 JSON object，并满足 descriptor schema 的 required 字段、基础类型、数字 `minimum`/`maximum` 约束、字符串 `minLength`/`maxLength` 约束与 `additionalProperties:false` 未知字段规则，`read_file.maxBytes` 当前要求 `>= 1`，标准工具 path/query/command/diff 字符串当前要求非空，required 为空的无参工具可把空参数 / `null` 归一为 `{}`，坏 JSON、非对象、缺 required 字段、基础类型错误、数值越界、字符串过短/过长或未知字段会写入 `invalid tool input:` 的 `tool_result`，不生成 `permission_request`，也不执行工具。当前 shipped tools schema 默认声明 `additionalProperties:false`，因此模型给出的额外字段不能被 `try?` 默认值吞掉后进入权限或工具执行。`CodeProjection` 根据 `tool_call_id` 将结果标题回填为 `result · <toolName>`，把 `tool error:` / `permission denied:` / `unknown tool:` / `invalid tool input:` 标成失败项，并通过 `RuntimeRecoveryAdvice` 派生恢复建议。GUI 与 CLI 均消费事件投影/observation，不解析 assistant transcript。
