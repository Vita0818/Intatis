...
- `task_create/update/get/list` 管理 durable WorkTask；coordinator 可管理当前 run/Goal 范围，worker 只能读自身/依赖并更新绑定且归属自己的 WorkTask。`task_create.owner` 可省略；若提供，必须来自较早 tool-call round 中成功的 `list_agents` / `spawn_agent` ToolResult 并且此刻已是 attached data-plane agent，不能预填计划中的未来名称。先建图再 spawn 时必须省略 owner，由 current caller 初始持有，待 spawn 成功后再用 `delegate_task` 原子转移 ownership；多 worker 只在同一阶段 batch（ownerless create → await → spawn → await → delegate confirmed pairs）。DAG 校验 missing/self/cross-run/cycle、readiness、revision 和 result/evidence；依赖变更由 host graph 在同一 batch 重新计算被编辑节点与下游 readiness，投影拒绝与 DAG 不一致的 ready/pending/blocked 事件。WorkTask 进入 `in_progress` 后执行契约字段不可改。`delegate_task` 可绑定 WorkTask 并追加 invocation linkage，但 child report 只是 candidate result；legacy child delegation 与 causal mailbox wake 未显式绑定 WorkTask 时，仍从父 TaskContract 继承 Goal/ContinuationRun scope。
- `task_update` 的 provider-facing business schema 按 capability 投影：`manage_work_tasks` 保持完整 manager schema；普通 worker 的 `update_owned_work_task` 保持同一稳定工具名，但 closed business schema 仅含 `task_id`、`expected_revision`、`progress_note`、`status`（`in_progress/blocked/completed/failed`）、`result`、`evidence`。automatic 模式仍在 request-owned provider copy 上单独装饰既有 authorization sidecar；它不是 WorkTask 业务字段。因此 contract、owner、DAG、priority、retry 与 cancel 不再成为 ordinary worker 可生成的业务参数；未知字段在 permission/executor 前拒绝。该投影不替代宿主对当前 task binding、ownership、revision、transition、result/evidence 的再次校验，也不改变 EventLog 或 concrete tool ID。
...
- WorkTask 的 production no-effect proof 同时覆盖 `task_create` 与 `task_update`，但仅覆盖各自首个 WorkTask EventLog append 前的窄 preflight scope。create 的 capability/run/title/owner/dependency/graph 拒绝可结算 `failed/not_started`；future/unattached owner 使用 `owner_not_attached` 引导模型省略 owner，或等待较早轮次的 spawn/list ToolResult 后重试。append、persistence failure 与 lost acknowledgement 仍是 unknown/manual reconciliation，不能因 create preflight 修复而放宽。
...
| `WorkTask` / `WorkTaskGraph` / `TaskEvidence` | 用户可见计划 DAG 节点；显式 acceptance criteria、expected artifacts、owner、result/evidence、revision 与 invocation linkage | `work_task_created/updated/owner_changed/dependency_changed/ready/started/progressed/blocked/completed/failed/cancelled/invocation_linked/evidence_added/carried_forward` append-only 事件 | stable `wt_` ID；`task_…` 是 AgentInvocation namespace，若 lookup 确认不存在则工具返回 typed no-effect 引导，但真实可查的 legacy 非标准 WorkTask ID 仍兼容。update 必须使用最新 authoritative revision，terminal task 不做冗余 settle；task_create/update 的 permission preview 只含 bounded、脱敏的语义字段，完整参数只以 digest/count 参与 revalidation。依赖只能在同一 ContinuationRun，拒 self/missing/cycle/stale revision；`in_progress` 后执行契约不可改；completed 必须有非空 result，存在 acceptance criteria 时还需 agent-reported evidence，但这些字段不能直接证明 Goal。新 run 由 host 原子取消并克隆旧非终态任务、重映射依赖；AgentInvocation 结果仅为 candidate |
