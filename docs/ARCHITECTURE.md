...
```text
EventLog raw message text (single source of truth)
  -> ConversationProjection / CodeProjection
  -> role policy
     -> user / system / structured special card: existing plain or dedicated view
     -> assistant / agent: IntatisMessageContentView(.richText)
        -> serial IntatisRenderDocumentWorker
        -> pinned cmark-gfm + GFM extensions + SOURCEPOS (before MarkdownContent)
           -> one read-only AST pass: complexity + CMARK_NODE_CODE/CODE_BLOCK ranges
              -> single-line CODE: direct SOURCEPOS mapping
              -> multiline CODE in containers: endLine + known backtick-count repair
           -> parse/unknown/sourcepos/complexity failure -> whole raw message plain
        -> math delimiter adapter (uses upstream code ranges; no handwritten backtick pairing)
        -> MarkdownUI 2.4.1 MarkdownContent (Markdown structure/layout)
           -> fenced code callback -> IntatisCodeBlockView
              -> serial worker -> official cdn-release highlight.js 11.11.1 via JavaScriptCore
              -> unknown/error/oversize -> selectable Menlo plain text
           -> explicit math URL -> route by stored presentation
              -> display / sole-inline promoted to block provider -> iosMath label view
              -> inline in prose -> labeled bitmap in MarkdownUI Text attachment
           -> remote Markdown image -> blocked placeholder
```

`IntatisMessageContentView` 是显示 adapter，不是第四个解析/排版引擎。Markdown AST/布局归 MarkdownUI，语法 token 分类和 grammar 归 highlight.js，TeX 语法与数学排版归 iosMath；Intatis 只维护角色策略、SwiftUI 容器、安全门与极薄的显式定界符路由。构造 `MarkdownContent` 前，`IntatisCommonMarkInspector` 以锁定的 `cmark-gfm`、`cmark-gfm-extensions`、与 MarkdownUI 同构的 GFM extension set（autolink/strikethrough/tagfilter/tasklist/table）和 `CMARK_OPT_SOURCEPOS` 解析一次 raw source；同一个只读 AST pass 同时计算复杂度，并从 `CMARK_NODE_CODE` / `CMARK_NODE_CODE_BLOCK` 取得 inline/block code ranges。单行 `CMARK_NODE_CODE` 直接映射 SOURCEPOS；多行 inline code 位于 blockquote/list 等容器时，cmark 报告的 `endColumn` 是剥离 continuation prefix 后的列，不能直接当 raw column。此时 adapter 只使用 cmark 已解析 node 的 `endLine` 与 `cmark_node_get_backtick_count`，在对应 raw 末行定位那个已知长度的 closing run；这属于有限 source-position repair，不搜索/配对 opener、不实现 code-span parser。找不到可证明的 closer/range 就整条 raw plain。该路径显式覆盖 LF、CR、CRLF；项目仍没有手写 backtick 配对器。数学 adapter 只在这些证明过的 ranges 之外识别 `\(...\)`、`\[...\]` 与 `$$...$$`；display pair 还必须 standalone 且位于 top level，container 内 display 保持 literal。因此 nested block/inline code、Unicode/multiline inline code、escaped/invalid/unclosed 输入不会被误转公式。代码块仍是带语言/Copy/Menlo/选择/双向滚动的完整只读容器。

AST complexity gate 位于任何 `MarkdownContent` 构造之前：每 paragraph 最多 128 个 soft/line break；每个 paragraph/heading/table-cell 最多 256 个非 break inline node；list nesting 最多 8；总 AST 最多 4,096 nodes；tree depth 最多 32。只要 raw source 含任一 math opener，就为最多 64 个 placeholder 的派生增长预留 256 nodes、1 层 depth 和每 container 192 个 inline nodes，所以本轮 raw-AST 实际预算分别收紧为 3,840 nodes、31 depth、64 non-break inline nodes。任何超限、parser/extension attach 失败、未知 node type 或不可信 source position 都整条返回 raw plain，不构造部分 rich tree；这只是上游 AST 上的 admission decision，不修改 AST、不重写 parser。

该 gate 对应 MarkdownUI 官方公开风险：[issue #312](https://github.com/gonzalezreal/swift-markdown-ui/issues/312) 的 soft-break crash、[#426](https://github.com/gonzalezreal/swift-markdown-ui/issues/426) 的中等长度/深层列表冻结、[#445](https://github.com/gonzalezreal/swift-markdown-ui/issues/445) 的约 10 层列表性能退化，以及尚未合并的 [PR #438](https://github.com/gonzalezreal/swift-markdown-ui/pull/438) 对递归 `Text + Text` 的分析。Intatis 不复制 PR #438 的未合并 renderer 核心修复，也不把 issue 中复现数字当平台稳定上限；只在进入上游 renderer 前 fail closed。

当前验证证据边界是 29 个 rendering tests 中可自动运行的 28/28 通过；唯一 highlight engine test 受 outer sandbox 的 AppKit HTML importer 限制，但同一真实 engine/HTML path 已由最新 Computer Use 通过。排除 Tools 与该 engine test 的 546 tests / 0 failures，以及跳过 engine 的全量 633 tests / 14 skipped / 34 failures（9 unexpected，全部为既有 Tools nested-sandbox/loopback）不能被表述为 29/29 automated pass。macOS/iOS generic build 与 Light/Dark、Copy、horizontal scroll、stage 1→2→3、sole-inline、display、fallback、production-root Computer Use 均通过。

安全与性能边界固定在项目 adapter 层：Markdown 图片 provider 不执行远程抓取；点击链接仅放行 `http`、`https`、`mailto`；JavaScriptCore 只加载已固定、已校验的官方 highlight.js 资源并调用其 `highlight`，不 `eval` 模型输出。由于 highlight.js 11.11.1 upstream open issue #4362 的 C/C++ quadratic `FUNCTION_DECLARATION` ReDoS，`c` / `cpp` 当前在调用 engine 前直接 plaintext fail-closed；完整代码框与复制不受影响，Intatis 不本地修改 grammar。代码 SwiftUI render revision 与 highlight cache 是分字段的结构化 key，language/source payload 以 UTF-8 `Data` 保持 byte identity；cache hit、HTML→attributed 转换后都按 UTF-8 `Data` 再验证，不能用可碰撞的字符串拼接或 Unicode canonical equality 代替。原始富文本 / 单公式 / 每消息公式定界符 candidate / 派生 Markdown / 单代码块高亮上限分别为 512 KiB / 32 KiB / 64 / 768 KiB / 64 KiB。完成文档缓存最多 96 项且总成本 4 MiB，高亮缓存最多 64 项；未完成消息的文档构建延迟 50 ms，代码高亮延迟 60 ms，SwiftUI task revision 更换时取消旧工作。MarkdownContent/cmark 同步解析与 highlight.js engine 调用各自进入 serial actor worker，避免阻塞 SwiftUI main actor；公式私有 URL 用内容 digest 作为 identity，避免流式内容变化命中旧 inline image。MarkdownUI 把整段唯一 inline image 提升到 block provider 时，provider 必须按存储的 `.inline` presentation 路由到 standalone inline math view，而不是远程图片阻断占位。display math 的 iosMath label view 暴露原始 TeX AX label；普通 prose inline bitmap 虽由 labeled `Image` initializer 构建，但 MarkdownUI `Text` attachment 在当前 macOS AX tree 不暴露该语义，因此 inline VoiceOver 仍是设备/上游验证缺口。inline math bitmap 在分配前限制为 1024 × 256 pixels 且最多 262,144 pixels，macOS 按 2x backing scale 预估（logical size 约 512 × 128）。消息/公式数量/派生文本超限、未知语言、高亮失败和数学预校验失败保留 raw text；inline renderer 本身失败时使用有界警告占位，不能宣称该路径绝对回退为屏幕上的原始公式，但 raw EventLog 始终未变；远程图片也使用安全占位。任一派生显示结果都不改 `EventLog`、`Envelope`、projection 或 provider 请求。
...
- 恢复期间 scheduler 保持暂停：先通过 `replayChecked` 验证 session identity、已知事件 payload、序号与 WAL recovery，再折叠 `tool_execution_prepared/settled`；锁/读取/损坏失败不得退化为空投影。普通 read-only execution 可随 running task 以 `attempt + 1` 重排；write/exec/network/destructive 或消息/委派/spawn/remove 等协作副作用若 prepared 未 settled，任务必须以 `manual reconciliation required` 失败，禁止自动重放。显式 retry 也必须重新 checked replay，不能因日志读取失败把 non-replayable 证据当空。created/assigned 半入队、超过 maxAttempts、缺关键 lease 的任务也明确失败；queued task、未消费 mailbox、task graph、lease history 与 token 用量从同一已验证事件快照重建。GUI/CLI 先恢复 `@main` 与 reviewer（CLI 仅在用户显式 `/default` 时允许人工模式），再启动 Goal recovery；只有 current Goal 的遗留 run 已取消/checkpoint/reconcile 后，普通 data-plane scheduler 才 resume。
- `GoalRuntimeController.start()` 在 Orchestrator/roster/reviewer 可用后观察 EventLog；仅当不存在 unresolved non-replayable execution 时，才处理 current Goal 下所有遗留 created/running ContinuationRun。start single-flight，pending stop 必须先重试；start 取消若已经创建 continuation，会先 scoped cancel、等待执行退出并 durable checkpoint，再返回 false。Goal mutation 与 ordinary turn 共享串行 mutation gate，stop 有独立 single-flight lock；shutdown 是终局 fence，不得在失败尾部 resume data plane 或接纳新 turn，延迟 watcher 事件必须重放最新 Goal revision/status 后才可 stop/launch。active Goal 先经过 scoped barrier；paused 等非 active Goal 对每个精确 Goal/run scope 建立 cancellation tombstone、取消 admission/execution，确认取消成功后才追加 recovered checkpoint，且不会自动执行 provider。每次进入下一 run 的 launch gate（包括进程内 Resume 与 active Goal Edit）都会顺序找出该 Goal 所有未审计 checkpoint，以 conservative continue audit 结算其 token/elapsed/blocker；若 checkpoint 对应 durable provider hard-limit，则结算为 usage-limited。任一取消、checkpoint、结算或持久化失败都不会创建新 run，避免 orphan checkpoint、重复 audit 和重复计费。Pause/Edit/Clear 必须先安全停止当前 run 并成功 durable checkpoint，失败时控制面变更 fail closed；Goal Edit 成功会清空旧 latest audit、blocker fingerprint、consecutive blocked/no-progress streak。completed/blocked/budget-limited/usage-limited 不自动续跑；active Goal 的连续 no-progress run 达阈值后保持 active 但停止自动循环，等待用户 steering/resume；默认阈值为 2，blocked 则要求同一 normalized blocker 至少连续 3 个 run。
- Goal continuation 每轮只由宿主建立一个 run；新 run 先原子 carry-forward 旧 run 的非终态 WorkTask，再向 `@main` 发送 `recordUserMessage=false` 的 scoped root invocation。carry-forward 会取消 source、为 target running run 分配新 WorkTask ID、重映射同批依赖并追加 `work_task_carried_forward`；session/Goal/run ownership 或外部非终态依赖不一致时 fail closed。scheduler barrier 与取消都只作用于同一 Goal/可选 run，并覆盖 queued、claimed、running execution；精确 Goal/run cancellation tombstone 在 root admission 入口、WorkTask carry-forward 后、admission lock 内和 durable queue append 后均检查，admission barrier 会等待在途创建，取消循环再 snapshot。已 durable 部分入队的 root task 必须先 settlement 为 cancelled，provider 才可保持未调用；terminal cancellation 落盘失败的 task 会被 quarantine 并从 result/idle waiters 的阻塞集合排除，但 tombstone 继续禁止 provider dispatch。run durable checkpoint 后才可 audit，每个 run 最多一次；production `settleGoalRunAudit` 在同一 admission lock/replay 下，以单个 append batch 提交 audit、run completed 与可选 Goal completed/blocked/budget-limited/usage-limited。verifier usage 以 scoped `turn_stats` 计入该 Goal/run。
...
| `EventLog` / `EventLogWriterLease` | session append-only 单写协调 | JSONL + `.lock` / `.writer.lock` / `.wal` / `.wal.tmp` sidecar | append/batch 在跨进程 exclusive flock 内重读尾 seq；多事件 batch 先同步并 rename WAL、同步父目录，再写/sync JSONL 与父目录，恢复按 exact prefix/suffix 判定 untouched/partial/full；所有 replay 在读取前先处理 WAL，兼容 replay 失败返回空，`replayChecked` / `isEmptyChecked` 对锁/读/known corruption/session mismatch fail closed；production Cowork runtime 全生命周期持有 writer lease，第二个 runtime fail closed |
...
| `ResolvedToolAuthorization` / `PermissionActionPreview` / `PermissionApprovalResolution` | 从同一 registry registration 解析的 concrete tool、canonical permission、membership、lease/target/args identity、审查语义与 typed outcome | authorization 作为可选追加字段复制到 permission review/resolved 与 tool execution events；preview 只保存有界脱敏字段；旧 JSONL 缺字段继续解码 | live automatic review 必须有完整 snapshot，审查后与 executor 前复核同一 authorization；旧事件只用于兼容 replay，不能在 live 路径重新解释 alias 或扩大 membership。`write_file` / `apply_patch` 均为 `filesystem.edit`；raw args 只由既有 tool-call 协议保存，automatic-review 只见 digest/count + preview |
...
| `ToolExecutionPreparedPayload` / `ToolExecutionSettledPayload` | 副作用执行票据、completion evidence 与 crash reconciliation | `tool_execution_prepared/settled` append-only 事件 | prepare 必须在 executor 前并携带/复核与 review 相同 authorization；仅普通 read-only 默认 safe-to-replay；write/exec/network/destructive 和 collaboration/lifecycle 工具默认 requires-manual-reconciliation。SideEffectEvidenceLedger 从 permission/review/resolved/prepared/settled 恢复证据，只有匹配的 succeeded settlement 才能清除完成阻断 |
| `TaskContract` / `TaskReportPayload`（AgentInvocation execution layer） | 单次 root/child agent invocation 契约与结构化回报；不是用户可见 WorkTask | `task_created/assigned/queued/started/completed/failed/cancelled` 既有事件；queue/start/terminal 记录 attempt | 契约记录 kind、issuer/assignee/objective/roleHint/deliverable/lease、reply mode、timeout、maxAttempts，并以可选字段绑定 WorkTask/ContinuationRun/Goal；普通用户消息与每个 Goal run 均创建 root invocation，legacy child/mailbox wake 继承 causal parent Goal/run scope。完成只产生 candidate result；追加字段保持旧 JSONL 可解码 |
...
- **Cowork automatic permission review**：brand-new GUI/CLI Cowork session 先用用户已选择的 primary workspace 受限 bootstrap 固定 `@main`，再创建 `@permission-reviewer`；两者之间不产生 `agent.attach` 模型审批。恢复的非空 session 会先恢复 durable roster，再重建 reviewer；若历史 session 异常缺少 `@main`，仍使用普通 reviewed attach。审查者固定 read_only、无工具 lease、`coordinationDepth=0`，不会启动嵌套 `AgentLoop`；`AgentPermissionResponder` 把宿主解析的 `ResolvedToolAuthorization`、参数 digest/count、有界秘密脱敏 action preview 与紧凑 causal/lease metadata 放入 untrusted block 后交给独立 FIFO/single-flight 控制面，不能根据 tool/capability alias 重新判断 membership。审查者只返回 `{"decision":"allow|deny","reason":"non-empty bounded reason"}`，且 risk 不得低于 gate；为 wire 兼容而收到 `ask_user` / `ask` 时规范化为 deny，不可解析输出、空 reason、tool call、timeout、cancel、provider error 或 settled persistence failure 也只以 typed failure durable deny 当前调用。GUI 不做隐式人工 fallback：reviewer 未就绪时 composer 锁定并提示 Retry；CLI `/auto` 重新启用，只有用户明确 `/default` 才移除审查者并恢复终端人工确认。
