...
Cowork session open/new -> CoworkProjectSettingsStore loads/saves per-session project settings
  -> replay EventLog -> restore roster/AgentInvocation graph/scheduler/mailboxes/leases/token usage
...
- 恢复期间 scheduler 保持暂停：先折叠 `tool_execution_prepared/settled`。普通 read-only execution 可随 running task 以 `attempt + 1` 重排；write/exec/network/destructive 或消息/委派/spawn/remove 等协作副作用若 prepared 未 settled，任务必须以 `manual reconciliation required` 失败，禁止自动重放。created/assigned 半入队、超过 maxAttempts、缺关键 lease 的任务也明确失败；queued task、未消费 mailbox、task graph、lease history 与 token 用量从 EventLog 重建。GUI/CLI 先恢复 `@main` 与 reviewer（CLI 仅在用户显式 `/default` 时允许人工模式），再启动 Goal recovery；只有 current Goal 的遗留 run 已取消/checkpoint/reconcile 后，普通 data-plane scheduler 才 resume。
...
- `ask_agent` / `delegate_task` 工具必须通过 scheduler 运行目标 agent，并通过 `MessageBus.deliver` / `Mediator` 把子任务结果返回给调用方；`ask_agent` 保持兼容的直接答案，`delegate_task` 的 `to` 可省略：优先复用同 workspace idle worker，不存在时在当前 task delegation budget 内原子创建 `worker-N`，再返回稳定 `task_id`、`agent_id` 与结构化 Task Report。并发省略 `to` 的 delegation 必须在任何 mediation/admission await 前预留其 worker，完成或失败时释放，不能让多个独立调用因 actor reentrancy 选中同一 idle agent。调用方不应只收到 queued ack，也不得通过直接嵌套 `AgentLoop` 取得结果。
