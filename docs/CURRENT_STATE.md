...
## 2026-08-02 macOS Icon Composer 图标与正式安装

- 根目录用户提供的 `Intatis.icon` 已作为 Apple Icon Composer 原始资源接入唯一发行
  target `IntatisMac`；`project.yml` 将它放入 resources build phase，并显式设置
  `ASSETCATALOG_COMPILER_APPICON_NAME=Intatis`。遗留 `IntatisMacAppStore` 与 iOS target
  均未接入本次图标。
