...
| `IntatisProtocol` | lib | Core | 结构化事件/线协议词汇（Event/Envelope/JSONRPC/Command/CoworkEvents/Goal/WorkTask/ContinuationRun/TaskGoalEvents/PermissionIntent/PermissionReview/ToolExecution/MultimodalEvents/TurnStats/JSONValue） |
...
| `IntatisConversation` | lib | Core, Protocol, Providers, Artifacts | 事件溯源会话 + UI 投影（跨进程锁定的 EventLog JSONL / ChatLoop / GoalInputParser / Projection / CodeProjection / Cowork Goal/WorkTask/ContinuationRun/tool-execution recovery projection / TurnStatsProjection） |
| `IntatisTools` | lib | Core, Protocol | 哑工具执行器与 task/goal host 接口（ToolProtocol/TaskGoalManagement/FileTools/PatchTool/PathConfinement/ShellGit/DocumentMediaTools/BrowserTools） |
...
- 事件日志与输入投影：`Packages/IntatisConversation/Sources/EventLog.swift`、`GoalInput.swift`（Chat/Code legacy Goal metadata 与 Cowork durable `/goal` 入口共用的语法解析）、`ExplicitGoalIntent.swift`（普通 Cowork 输入的窄、确定性中英文持续 Goal intent；不读取附件）、`Projection.swift`、`CodeProjection.swift`、`CoworkProjection.swift`
