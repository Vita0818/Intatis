...
| `IntatisProtocol` | lib | Core | 结构化事件/线协议词汇（Event/Envelope/JSONRPC/Command/CoworkEvents/PermissionReview/ToolExecution/MultimodalEvents/TurnStats/JSONValue） |
...
| `IntatisAgentKernel` | lib | Core, Protocol, Providers, Tools, Permission, Conversation, Artifacts | 单 agent 工具循环（Agent/AgentLoop/ContextBuilder/ContextProjector/AgentExecutionBudget/PermissionResponder） |
...
- Cowork 链路：`Packages/IntatisCowork/Sources/Orchestrator.swift`（生产 `runtime` session writer lease、真实 root task、bounded scheduler/watchdog、non-idempotent crash reconciliation、durable-first detach/revoke、恢复/取消/超时/重试、mailbox wake、lease lifecycle、Task Report、idle recycle）、`AgentScheduler.swift`（queue/claim/single-flight/snapshot/mailbox）、`AgentPermissionResponder.swift`、`PermissionReviewControlPlane.swift`（reviewer 独立 FIFO/single-flight/timeout/budget/durable verdict）、`MessageBus.swift`、`Mediator.swift`、`CoordinatorTools.swift`、`AskAgentTool.swift`
- Cowork 任务协议/投影：`Packages/IntatisProtocol/Sources/Task.swift`、`TaskGraph.swift`、`Leases.swift`、`CoworkEvents.swift`、`PermissionReview.swift`、`ToolExecution.swift`、`Event.swift`（task attempt/cancel、permission review request/settled、tool execution prepare/settled、message consumed、lease revoke）、`Packages/IntatisConversation/Sources/CoworkProjection.swift`（任务/agent roster/mailbox/owner/report/attempt/retry/未决 tool execution 投影）、`EventLog.swift`（append-only replay、cross-process flock、batch append、session writer lease 与 crash-tail 防护）
- Agent 内核：`Packages/IntatisAgentKernel/Sources/AgentLoop.swift`（严格 completion/finish reason、精确 permission review context、audit fail-closed、tool execution ticket、workspace lease、并行 collaboration calls）、`AgentExecutionBudget.swift`（dispatch 前 reservation 的共享 soft token meter）、`Agent.swift`、`ContextBuilder.swift`（untrusted user-role context）、`ContextProjection.swift`（task-scoped bounded projection）、`PermissionResponder.swift`、`ProviderImageGenerationToolService.swift`
