...
| `IntatisProtocol` | lib | Core | 结构化事件/线协议词汇（Event/Envelope/JSONRPC/Command/CoworkEvents/Goal/WorkTask/ContinuationRun/TaskGoalEvents/PermissionIntent/PermissionReview/ToolAuthorization/ToolExecution/MultimodalEvents/TurnStats/JSONValue） |
...
| `IntatisSharedUI` | lib | Core, Protocol, Providers, Conversation, Artifacts, MarkdownUI 2.4.1, cmark-gfm + cmark-gfm-extensions（swift-cmark 0.5.0）, iosMath 2.5.0 | 跨平台 SwiftUI；`MessageRendering` 负责 assistant/agent Markdown、代码块与 TeX 显示；同构 pinned GFM parser 在 MarkdownContent 前一次 AST pass 同时提供 CODE/CODE_BLOCK ranges 与 complexity gate，多行 inline code 仅对 cmark 容器前缀 endColumn 做有证据的 source-position repair；系统 JavaScriptCore 调用官方 highlight.js 11.11.1 |
...
`IntatisCoreTests`、`IntatisProtocolTests`（+V02/V03/V04）、`IntatisProvidersTests`（+Multimodal/ToolCalling）、`IntatisArtifactsTests`、`IntatisConversationTests`（+Code）、`IntatisToolsTests`、`IntatisPermissionTests`（+Reviewer）、`IntatisAgentKernelTests`、`IntatisCoworkTests`（含 `AutomaticPermissionReviewTests`）、`IntatisMultimodalTests`、`IntatisSharedUITests`（`MessageRenderingTests`）。`swift test` 无头。
...
- 权限门：`Packages/IntatisProtocol/Sources/PermissionIntent.swift`、`ToolAuthorization.swift`（宿主解析的 concrete tool/membership/lease/args digest/preview 不可变快照）、`Packages/IntatisPermission/Sources/PermissionEngine.swift`、`DeterministicPolicyGate.swift`、`ModelPermissionReviewer.swift`、`SecretScanner.swift`；production Cowork 的唯一模型审查点是 `PermissionReviewControlPlane` responder，审查者只消费宿主已解析的 membership 与语义风险，不能重新解释 capability alias；非 Cowork host 才可选择 in-engine reviewer
...
- 对话内容渲染：`Packages/IntatisSharedUI/Sources/MessageRendering/IntatisMessageContentView.swift`（角色策略、MarkdownUI 主入口、远程图片/链接策略、流式防抖，先显示 raw source 再接收 worker 结果）、`IntatisRenderDocument.swift`（serial actor 中、在 `MarkdownContent` 前使用 pinned `cmark-gfm` + GFM extensions + SOURCEPOS；一个只读 AST pass 同时计算复杂度并提取 `CMARK_NODE_CODE` / `CMARK_NODE_CODE_BLOCK` ranges；单行 inline 直接映射 SOURCEPOS，多行 inline 用 cmark node 的 `endLine` + `backtick_count` 在对应 raw 末行定位已知长度 closer，以修正 blockquote/list continuation prefix 导致的 endColumn 偏移，不能证明即整条 plain；兼容 LF/CR/CRLF，且不配对 opener、不实现 code-span parser；parse/unknown/sourcepos/超限同样整条 raw plain）、`IntatisCodeBlockView.swift`（完整代码框、Menlo、语言标签、复制、滚动；结构化 UTF-8 `Data` key + byte-exact 复核；highlight.js 11.11.1，C/C++ 因 #4362 plaintext）、`IntatisMathView.swift`（iosMath inline/display、sole-inline route、bitmap cap、display AX label；普通 inline VoiceOver 待验证）、`Resources/`（官方 highlight.js + HighlighterSwift a11y 主题）。AST 门限是 breaks 128、inline/container 256、list 8、nodes 4,096、depth 32；math opener 为 64 placeholders 预留 node 256/depth 1/inline 192。接入点为 macOS `IntatisChatScreen.swift` 与共享 `{Views,CodeViews}.swift`；`MessageRenderingTests` 仍为 29 项/可自动运行 28/28，其中 Unicode/multiline inline 测试现覆盖 blockquote+list continuation 末行公式及 LF/CR/CRLF。DEBUG fixture、provenance、上游 #312/#426/#445/#438 风险与验证边界保持前述记录。
- 渲染最终验证基线：排除 `IntatisToolsTests` 与 outer-sandbox 不可运行的 AppKit HTML importer engine test 后 546 tests / 0 failures；跳过 engine test 的全量执行 633 tests、14 skipped、34 failures（9 unexpected），均在既有 Tools nested `sandbox-exec` / loopback；macOS 与 iOS generic build 通过；最新 Computer Use 通过 Light/Dark、Copy、水平滚动、stream stage 1→2→3、sole-inline、display、fallback 与 production root。不得把这组证据改写成 29/29 automated pass。
...
- GUI/CLI Cowork 恢复与自动权限审查：`Apps/IntatisMac/Sources/CoworkViewModel.swift`（session replay → orchestrator restore，并保持 persistent scheduler suspension → reviewer/main bootstrap → GoalRuntime start/recovery/reconcile → 显式 `resumePendingTasks`；普通 turn scoped run、durable `/goal`、Goal/WorkTask projection、pause/resume/edit/clear、permission FIFO、cancel/retry、async stop 先 checkpoint Goal 再 `cancelAll`）、`Apps/IntatisMac/Sources/IntatisMacApp.swift`（CoworkShell Goal/Tasks binding、Goal edit sheet、clear confirmation）、`Apps/IntatisMac/Sources/IntatisMacRootView.swift`（session/mode transition generation guard，切换前等待旧 Cowork stop）、`Apps/intatis-cli/Sources/Interactive.swift`（per-workspace durable Cowork log；同样先 restore/GoalRuntime start/recovery，最后才显式 resume data plane；`/goal <objective>|status|pause|resume|clear|edit`、退出/切 mode 前 Goal checkpoint + `cancelAll`、`/auto` / `/default`、Code/Cowork image-generation service 注入）、`Apps/intatis-cli/Sources/Terminal.swift`（渲染 `permission_review`）
- Cowork macOS project mode：`Apps/IntatisMac/Sources/CoworkProjectSettings.swift`（per-session settings store + settings sheet）、`Apps/IntatisMac/Sources/CoworkViewModel.swift`（恢复后先注册 EventLog stream；fresh session 先 restricted bootstrap `@main`、再启用 reviewer；`projectSettings` / `project` 与 durable Goal/WorkTask projection、GUI no-mention 默认路由到 `@main`、project directory metadata add/remove）、`Apps/IntatisMac/Sources/Workspace.swift`（workspace picker prompt + bookmark restore）、`Packages/IntatisSharedUI/Sources/CoworkViews.swift`（仅在实际 `@main` 已投影时启用 composer；宽屏 `Git Status` / `Agents` / `Goal` / `Tasks`，窄屏 Goal/Tasks dock；Goal pause/resume/edit/clear 与 WorkTask criteria/result/evidence/dependency/invocation detail；默认不提供用户手动新建 agent 入口）
