...
- 已删除的 `IntatisMacAppStore` target/scheme、`INTATIS_MAC_APP_STORE` 编译条件和
...
- 当前产品平台链接必须保持：Developer ID macOS 与 CLI 可链接 stdio + HTTP；
  ...
  `IntatisMacAppStore` HTTP-only 产品图。
...
- **Cowork 文档/媒体工具 lease**：read-only worker 默认获得 in-process `read_pdf`、五个 exact process-backed reader capability（`readDOCX` / `readPPTX` / `readXLSX` / `readHTML` / `readEPUB`）与 `documentOCR`；五个普通 reader 必须携带 exact `execution_class=structured_read_only` intent，只声明 read/execute，且为 `.safeToReplay`，不得写工作区、请求网络或借此获得通用 shell。该 existing exact 只读集合由 deterministic gate 默认放行，不创建权限请求或调用 reviewer；解析失败必须写 failed settlement、作为 observation 返回并允许同批其他读取继续。`document_render`、`document_export_pdf`、`document_write`、`compile_latex`、`generate_image`、`edit_image` 等写入/网络能力只向 read-write worker/coordinator 显式签发。格式 reader capability 必须保持拆分。
