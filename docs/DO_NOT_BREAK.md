...
- automatic ask-class exact call 的 authorization reporter 必须复用 requesting agent 当前 exact provider/model 与刚才的 provider-facing conversation snapshot；request 只能暴露一个未注册、永不执行的 output-only `submit_permission_authorization` function，且不得同时暴露任何业务工具。该链路不得依赖上游支持 forced `tool_choice` 或 `response_format`；宿主只接受无 prose、恰好一个同名 function call，并严格解析其参数。缺失、错名、多个 call、混入文本或 malformed arguments 均 fail closed。不得借 reviewer provider、回退 current/default binding、进入普通 AgentLoop/scheduler/TaskGraph/UI/model history，或跨 tool call/cache 复用报告。
