...
- **Cowork mailbox 消费/丢弃协议**：typed message 必须先通过 Mediator 并成功追加 EventLog，才能进入 runtime mailbox。每个 new mailbox delivery `TaskContract` 必须冻结 1–8 个去重 exact MessageID；ContextProjector、presented IDs 与 consumed 必须依次收窄为该集合。ordinary mailbox task 只能获得 read-only workspace、reply-only communication、无 delegation/MCP/写入/终端/Git/browser 的窄 lease；`request_delegation` 也只能在原 ceiling 内获得最多一次、深度 1 的 delegation。成功 task terminal、可选 candidate WorkTask progress 与全部 exact consumed events 必须在一个 EventLog batch 落盘，成功后才从 runtime mailbox ack；append 失败不得留下 completed/consumed 半状态。若 owning Goal/run 在呈现成功前取消，只能 durable `agent_message_discarded` 后 ack，不能伪造 consumed。不得确认未呈现的下一批、在失败轮清空 mailbox，或在 replay 时复活已 consumed/discarded 消息。delivery 失败只能对同一 TaskID 做受 `maxAttempts` 约束的有界重试；已耗尽或 side-effect-quarantined 的 MessageID 必须保持 pending，不能换 TaskID 重置 attempts，但也不能阻塞后来未绑定的 ID。legacy nil binding 只能按 exact recipient/issuer/causal relatedTasks/Goal-run/时间 lineage 保守匹配；歧义或 lineage 任一 task 已耗尽时 fail closed。
