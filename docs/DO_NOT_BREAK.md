...
- **WorkTask DAG、owner 与 revision**：`WorkTaskGraph` 必须拒绝缺失/self/cross-run/cyclic dependency、stale revision、非法状态转换、未满足依赖，以及缺少必需 result/evidence 的完成；依赖重规划必须由 host graph 在同一 durable batch 重新计算被编辑节点及下游 readiness，projection 不得接受与 DAG 不一致的 ready/pending/blocked 事件。进入 `in_progress` 后 title/description/criteria/expected artifacts/priority/dependencies 等执行契约不得再改。`task_create.owner` 若存在必须是较早成功 ToolResult 已确认、当前 attached 的 data-plane agent；planned/future agent 不得预填。spawn 前建图必须省略 owner，等待 create 与 spawn 各自成功后才可用 `delegate_task` 转移 ownership；同一 assistant response 不能用后一个 call 的结果满足前一个或同批依赖 call。stable `wt_` ID 和 latest authoritative revision 不能被 UI index 或 AgentInvocation `task_…` ID 替代；确认不存在的 `task_…` lookup/update 必须给 typed no-effect namespace 引导，但真实存在的 legacy 非标准 WorkTask ID 仍可使用。`task_create/update` 的 reviewer preview 只能包含 bounded、脱敏语义字段，完整执行参数只用 digest/count 绑定 authorization。`task_create/update/get/list` 必须走 strict schema、PermissionIntent、CapabilityLease 与 durable EventLog，不能原地修改 projection。
...
- **WorkTask / Goal capability lease**：coordinator 可持有 `manage_work_tasks` / `create_goal`，worker 默认只有 `read_work_tasks` / `update_owned_work_task` / `read_goal`。worker 只能读取当前绑定 WorkTask、其依赖和当前 Goal，并更新自己拥有且绑定的当前 WorkTask；不得改 DAG、owner、priority、retry/cancel 或提交 Goal verdict。ordinary worker 的 provider-facing `task_update` 必须保持 closed narrow business schema，只暴露 `task_id`、`expected_revision`、`progress_note`、允许的 `status`、`result`、`evidence`；automatic request-owned authorization sidecar 仍按既有规则另行装饰，不得借此把 manager 的合同/图/归属/优先级/重试/取消字段重新暴露给 worker。manager 的完整 schema 与宿主二次校验必须保留，同名投影不得被误当成新的 EventLog 协议或 concrete tool identity。`submit_goal_verdict` 只能进入 exact `@main` 的 current default/task lease 或受控 host seam；普通 worker、spawn coordinator、task-scoped non-main 与 reviewer 必须显式移除，legacy main 只能 durable replace default 而不能原地扩大历史 lease。即使 `@main` 调用 `update_goal`，complete 仍必须消费独立 GoalVerifier + host-derived validation audit，blocked 仍要求既有三轮 blocker proof；工具本身不得创建 audit 或扩大终态条件。
...
- **WorkTask create 的 proof scope**：production `task_create` 与 `task_update` 都只能由 Orchestrator adapter 在首个 WorkTask EventLog append 前证明 no-effect。create 的 capability/run/title/owner/dependency/graph rejection 可转为 typed `not_started`；append/persistence/lost-ack 必须留在 proof scope 之外并保持 unknown/manual。不得把这项修复扩展成按 `IntatisError` case 或错误字符串做全局重试推断。
