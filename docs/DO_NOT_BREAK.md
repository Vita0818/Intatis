...
- **Cowork 文档/媒体工具 lease**：read-only worker 默认获得 in-process `read_pdf`，并可获得固定 process-backed observation 的 `document_read` / `document_ocr`；后两者必须携带 exact `execution_class=structured_read_only` intent，只声明 read/execute，不得写工作区、请求网络或借此获得通用 shell。`document_render`、`document_export_pdf`、`document_write`、`compile_latex`、`generate_image`、`edit_image` 等写入/网络能力只向 read-write worker/coordinator 显式签发。五个 process 文档 capability 必须保持拆分，不能因为 `ToolRegistry.standard()` 或某一项授权而泄漏其他项；legacy `readDocument` / `editPDF` / `reconstructDocument` raw values 永不签发。
...
- 任意 host 缺 structured process 能力时，process-backed
  `document_read` / `document_ocr` / `document_render` / `document_export_pdf` /
