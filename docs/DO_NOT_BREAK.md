...
- 自动阈值只能来自 exact route 的显式 context metadata：默认 total-scope 90%
  ...
  时）。summary output request ceiling 当前不得超过 4,096 tokens，host 还
  必须在 append stream delta 前执行对应的实际输出 bound，并在 replacement
  前拒绝 `SecretScanner` 命中的摘要；provider 忽略 ceiling、逻辑裁剪破坏
