...
- **Chat web-search citation 兼容性**：`message_completed.citations` 只能作为 optional additive 字段演进，旧 JSONL 缺字段必须继续解码。citation 必须来自 provider 的结构化 URL annotation，只接受带 host、无 user-info 的 HTTP(S) URL并按 URL 去重；UI 创建 `Link` 前必须再次验证。不得从 assistant Markdown 猜测 citation，也不得把 citations 复用为本地浏览器权限、Tool call、自动打开页面或 iOS Tools/AgentKernel linkage。开启搜索的 Chat request 必须显式走支持 hosted web search 的 Responses route；失败不得静默降级成未搜索的 Chat Completions 回答。
- **Chat web-search route 兼容性**：外部配置的可选 `web_search_model` 必须与正常 `model` 使用同一 exact provider/model 解析合同，省略才允许复用当前 Chat route；不得把“省略”持久化成会随选择漂移的复制字符串。显式无效 provider/model、无效 `responsesEndpoint` 或不支持 Responses hosted search 必须 fail closed，不能静默切换 provider/model 或关闭搜索。该 route 只属于配置文件/导入 snapshot，不得被普通 Settings 保存流程、当前 Chat 模型切换或 UserDefaults catalog 重建丢失。Chat 必须透明提供搜索并让 provider 以 `tool_choice: auto` 按需决定，macOS/iOS 均不得新增搜索按钮、菜单项、开关、状态或发送目标提示，也不得在 UI 泄露 endpoint、credential。
...
- **Cowork 文档/媒体工具 lease**：worker 默认只能获得安全的 in-process 只读文档能力（当前为 `read_pdf`）；process-backed `read_document` 与 `edit_pdf_pages`、`reconstruct_document_image`、`compile_latex`、`generate_image` 等写入/执行/网络相关工具必须只通过 read-write coordinator/worker 的显式 `CapabilityLease` 授予，不能因调用 `ToolRegistry.standard()` 而泄漏给 read-only worker。
