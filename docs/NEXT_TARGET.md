...
## 目标：完成 v0.38 Developer ID 直接分发候选

把当前 v0.38 源码收敛为可验证、可安装、可直接分发的 macOS ZIP/DMG；不走 App Store。
...
- Xcode 工程已重新生成；macOS universal Release 与 iOS Simulator Debug 均构建通过，最终
  bundle 均为 `0.38 (38)`，macOS 可执行文件包含 `x86_64 arm64`。本机当前已安装的 ad-hoc
  Hardened Runtime 开发构建仍为 `0.36 (36)`，不能作为 v0.38 Developer ID 公证发行证据。
...
- 不把历史 v0.10/v0.16 文档批量改名为 v0.38；它们是历史证据。
