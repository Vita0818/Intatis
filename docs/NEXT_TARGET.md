...
- Apple 已接收两次 App submission，但查询时均长时间保持 `In Progress`；旧脚本的无输出
  `--wait` 已被用户中断，且旧临时 App 已清理。发行脚本现改为显示 upload/status、保存
  submission ID、默认 30 分钟有界等待，并在超时/中断时保留可恢复签名 App/DMG。

## 剩余 release gate

1. 先用 `xcrun notarytool history --keychain-profile Intatis-Notary` 查看现有两条 submission；
   在它们仍为 `In Progress` 时不要继续上传。
