...
当前 `HEAD` / `origin/main` 的最新里程碑提交为 `v0.32`，因此本轮把长期停留在
`0.12 (1)` 的工程元数据校准为 `0.32 (32)`。build 32 用于结束此前所有构建都复用
build 1 的歧义；以后构建号必须单调增加，但不要求永远等于 marketing version 的 minor。
